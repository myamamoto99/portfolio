const navStyles = {
  ...
  menu: "absolute right-0 top-full z-40 mt-2 flex min-w-[11rem] flex-col rounded-xl border border-[var(--ui-border-soft)] bg-[var(--ui-surface-base)] p-1 shadow-[0_18px_40px_-24px_var(--ui-shadow-elevated)] md:hidden",
  itemBase:
  ...
  menuItemActive: "bg-[var(--ui-surface-subtle)] text-[var(--ui-accent)]",
  menuItemInactive: "text-[var(--ui-text-muted)] hover:bg-[var(--ui-surface-subtle)]",
} as const;
...
const getMenuItemClassName = (isActive: boolean) => {
  return `${navStyles.menuItemBase} ${isActive ? navStyles.menuItemActive : navStyles.menuItemInactive}`;
};

export { navStyles, getNavItemClassName, getMenuItemClassName };
