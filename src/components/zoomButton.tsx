...
export default function ZoomButton({ zoomType }: { zoomType: "in" | "out" }) {
    return (
        <button
            ...
            onClick={() => {
                const imageElement = document.querySelector(`.${bakingStyles.modalImage}`) as HTMLImageElement;
                if (imageElement) {
                    const currentScale = imageElement.style.transform ? parseFloat(imageElement.style.transform.replace(/scale\((.*)\)/, "$1")) : 1;
                    const newScale = zoomType === "in" ? currentScale + 0.2 : Math.max(currentScale - 0.2, 0.5);
